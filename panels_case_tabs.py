"""
Sharelock v2 — Right-panel tab builders (Rule-6 split of panels_case.py).

Analysis / Gap Review / Graph / Report tab content + the shared
folder→case resolution and ``ctx.cache`` case-summary loader. The
``@ext.panel`` entrypoint, tab dispatch and share/settings wiring stay in
``panels_case.py`` (which imports the builders from here — one direction,
no cycle).
...
from app import _user_id, _user_agency
import queries
